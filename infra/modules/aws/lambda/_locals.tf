locals {
  source_code_hash = {
    ...
    twitch_event_sub_webook = fileexists("${path.module}/bin/twitch-event-sub-webhook/bootstrap.zip") ? filebase64sha256("${path.module}/bin/twitch-event-sub-webhook/bootstrap.zip") : data.aws_lambda_function.twitch_event_sub_webhook_exists[0].source_code_hash
