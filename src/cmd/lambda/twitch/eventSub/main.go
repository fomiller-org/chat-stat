...
func HandleRequest(ctx context.Context, event DynamoDBEvent) {
	...
	for _, record := range event.Records {
		...
		// err := dynamodbattribute.Unmarshal(record.Change.NewImage, &myItem2)
		// if err != nil {
		// 	fmt.Printf("Error UnMarshaling MyDBItem: %s", err)
		// }
		// fmt.Printf("Item: %v\n", myItem2)
		//
		recordData, err := dynamodbattribute.MarshalMap(record.Change.NewImage)
		if err != nil {
		...
		var myItem MyDBItem
		err = dynamodbattribute.UnmarshalMap(recordData, &myItem)
		if err != nil {
			fmt.Printf("Error UnMarshaling MyDBItem: %s", err)
		}
		fmt.Printf("Item: %v\n", myItem)
